...
describe('selectStreamSource', () => {
  ...
  it('falls back to the first remaining source when the requested id rotated away', () => {
    // A re-scrape rotated the signed URLs, so the id the player minted no
    // longer exists — serve the ordered first source (all formats proxy).
    const list = [src('a', 'mpd'), src('b', 'mp4'), src('c', 'mp4')];
    const selected = selectStreamSource(list, 'stale-id');
    expect(selected?.id).toBe('a');
  });

  it('serves adaptive manifests through the proxy like any other source', () => {
    const list = [src('a', 'mpd'), src('b', 'mp4')];
    expect(selectStreamSource(list, 'missing')?.id).toBe('a');
  });
