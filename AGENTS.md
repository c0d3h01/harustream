...
- Always read `.agents/skills/` (gitignored) at the start of a task and follow any skill whose description matches the work — the set currently includes `vercel-react-best-practices` (React/Next perf rules), `systematic-debugging`, `next-dev-loop`, and more. When a matching skill exists, follow it.
