...
import type { EpisodeLink, ProviderContext, Stream } from '../types';
...
export const getStream = async ({
  ...
  try {
    const modGetEpisodeLinks = async ({
      url,
      providerContext,
    }: {
      url: string;
      ...
      if (url.includes('url=')) {
        ...
        const res2 = await axios.get(newUrl || url, { signal });
        const html2 = res2.data;
        $ = cheerio.load(html2);
      }
      const episodeLinks: EpisodeLink[] = [];
      $('h3,h4').each((_i, element) => {
    ...
    const downloadLink = await modExtractor(url, providerContext, signal);
    ...
    const driveRes = await axios.get(driveLink, { headers, signal });
    const driveHtml = driveRes.data;
    const $drive = cheerio.load(driveHtml);

    try {
      const resumeBot = $drive('.btn.btn-light').attr('href') || '';
      const resumeBotRes = await axios.get(resumeBot, { headers, signal });
    ...
    try {
      ...
      const cfWorkersRes = await axios.get(cfWorkersLink, { headers, signal });
...
async function modExtractor(url: string, providerContext: ProviderContext, signal?: AbortSignal) {
  const { axios, cheerio } = providerContext;
  try {
    ...
    bodyFormData.append('_wp_http2', wpHttp2?.toString() || '');
