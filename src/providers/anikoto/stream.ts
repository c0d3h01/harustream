...
import type { ProviderContext, SkipInterval, Stream, TextTracks } from '../types';
...
export const getStream = async ({
  ...
  try {
    ...
    if (!dataIds) {
      const detailRes = await axios.get(watchUrl, { headers, signal });
      const $d = cheerio.load(detailRes.data);
      const animeId = $d('#watch-page, #watch-main, .watch-wrap, [data-id]')
        .first()
        .attr('data-id');
    ...
    await Promise.all(
      tasks.map(async (task) => {
        try {
          ...
          const skipTimings = await providerContext.kvStore?.get<boolean>('anikoto_skipTimings');
          const skipTimingsEnabled = skipTimings ?? true;
          let skipIntervals: SkipInterval[] | undefined;
