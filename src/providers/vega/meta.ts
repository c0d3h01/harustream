...
import { addCinemetaContext, getCinemetaMeta, getCinemetaSeason } from './cinemeta';
...
function applyCinemeta(info: RawInfo, meta: any): RawInfo {
  return {
    ...info,
...
export async function getMeta({ link, signal, ctx }: FetchMetaArgs): Promise<RawInfo> {
  try {
    ...
    const url = new URL(link, `${providerBaseUrl('vega')}/`).href;
    ...
    const synopsisHeader = $('h3').filter(
      (_i, el) => $(el).text().includes('SYNOPSIS/PLOT') || $(el).text().includes('Plot'),
    );
    if (synopsisHeader.length > 0) {
      synopsis = synopsisHeader.next('p').text().trim();
    }
    if (!synopsis) {
      const synopsisNode = infoContainer?.find('p')?.next('h3,h4')?.next('p')?.[0]?.children?.[0];
    ...
    const cinemeta = await getCinemetaMeta(imdbId, type, ctx);
    ...
    throwProviderError('Vega', 'meta', err);
  }
}
