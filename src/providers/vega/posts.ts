...
export async function getPosts({
  ...
  try {
    ...
    const url = filter
      ...
      : `${BASE_URL}/page/${page}/`;
    return parsePosts(BASE_URL, url, signal ?? new AbortController().signal, headers, axios, cheerio);
  } catch (err) {
    console.error('Vega getPosts error:', err);
    return [];
  }
}
...
export async function getSearchPosts({
  ...
  try {
    ...
    console.error('Vega getSearchPosts error:', err);
    return [];
  }
}