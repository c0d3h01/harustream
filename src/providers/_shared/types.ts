// ─── Provider Types ─────────────────────────────────────────────────────────
// All types used by provider modules. Import from here in provider code.

import type { AxiosInstance } from 'axios';
import type * as cheerio from 'cheerio';
import type { Catalog, SkipInterval } from '@/types';

export type { Catalog, SkipInterval };

export interface ProviderUrlsEntry {
  name?: string;
  url?: string;
  /** True when the provider has shut down or been abandoned (no probing). */
  retired?: boolean;
  /** Why the provider is retired — kept as a record. */
  retiredNote?: string;
}

export type ProviderUrls = Record<string, ProviderUrlsEntry>;
...
export interface ProviderModule {
  ...
  getSettingsSchema?(args: { ctx: ProviderContext }): Promise<SettingsField[]>;
}
