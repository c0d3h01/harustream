...
import type { ProviderUrls, ProviderUrlsEntry } from './types';

function entry(providerId: string): ProviderUrlsEntry {
...
/** True when the provider is marked retired (shut down / abandoned) in urls.ts. */
export function isProviderRetired(providerId: string): boolean {
  return entry(providerId).retired === true;
}
