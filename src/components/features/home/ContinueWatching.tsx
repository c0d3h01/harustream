        ...
        <AnimatePresence>
          {items.map((entry, index) => {
            ...
            return (
              <motion.div
                key={entry.link}
                className="group/card relative w-[clamp(110px,30vw,150px)] shrink-0"
                initial={{ opacity: 0, y: 10 }}
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, scale: 0.92, transition: { duration: 0.18, delay: 0 } }}
                transition={{
                  duration: 0.3,
                  delay: index * 0.05,
                  ease: [0.16, 1, 0.3, 1],
              ...
              >
                ...
                {/* Per-card clear: siblings of the resume button so they never
                    trigger playback; a 44px hit target (touch-target-style)
                    with a smaller visible chip. Shown on hover/focus for fine
                    pointers and always for coarse (touch) ones. */}
                <button
                  type="button"
                  aria-label={`Remove ${entry.title || 'Untitled'} from continue watching`}
                  onClick={() => clear(entry.link, entry.episode)}
                  className="absolute top-0 right-0 z-10 grid size-11 place-items-center rounded-full opacity-0 transition-opacity duration-200 group-hover/card:opacity-100 focus-visible:opacity-100 focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-ring motion-reduce:transition-none pointer-coarse:opacity-100"
                >
                  <span className="grid size-7 place-items-center rounded-full bg-background/75 text-foreground shadow-xs backdrop-blur-sm transition-colors group-hover/card:bg-background/90">
