...
function TitleHeaderInner({ item, canPlay, loadingSources, onPlay }: TitleHeaderProps) {
  ...
  const posterAlt = useMemo(() => t('home.posterAlt', { title: item.displayTitle }), [item.displayTitle, t]);
  ...
  return (
    <section aria-labelledby="title-heading">
      <div className="flex gap-4 sm:gap-6">
        <div className="relative aspect-2/3 w-28 shrink-0 overflow-hidden rounded-xl border border-border/70 bg-secondary sm:w-36 lg:w-44">
          {item.posterUrl ? (
            ...
            <div className="absolute inset-0 grid place-items-center text-muted-foreground" aria-hidden="true">
      ...
      {item.synopsis ? (
        <p className="mt-4 max-w-2xl text-sm leading-relaxed text-muted-foreground" id="title-synopsis">
          {item.synopsis}
        </p>
      ) : null}
